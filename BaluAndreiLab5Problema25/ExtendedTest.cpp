...
void testVerkettung(Relation r1, Relation r2) {
	cout << "Test Verkettung\n";
	SortedMultiMap smm1 = SortedMultiMap(r1);
	SortedMultiMap smm2 = SortedMultiMap(r2);
	SortedMultiMap smm3 = SortedMultiMap(r1);

	populateSMMEmpty(smm1, 1, 10);
	assert(smm1.size() == 15);

	populateSMMEmpty(smm2, 1, 10);
	assert(smm2.size() == 15);

	populateSMMEmpty(smm3, 1, 10);
	assert(smm3.size() == 15);

	try {
		smm1.verkettung(smm2);
		assert(false);
	}
	catch (exception&) {
	}
	
	smm1.verkettung(smm3);
	assert(smm1.size() == 30);
	assert(smm3.size() == 15);
}
