...
void testStack() {
	...
	for (int idx = 0; idx <= max; idx++) {
		stack.push(idx);
		assert(stack.size() == idx + 1);
	}
	assert(!stack.isEmpty());
	for (int idx = 0; idx <= max; idx++) {
		assert(stack.top() == max - idx);
		assert(stack.pop() == max - idx);
	}
	assert(stack.isEmpty());
}

void testIteratorInBothDirections(Relation rel) {
	cout << "Bidirectional Iterator Tests\n";
	...
	SortedSetIterator it = s.iterator();
	int iverif = 0; int vverif[5]; TComp e;
	it.last();
	e = it.getCurrent();
	vverif[iverif++] = e;
	it.prev();
	while (it.valid()) {
		assert(rel(it.getCurrent(), e));
		e = it.getCurrent();
		vverif[iverif++] = e;
		it.prev();
	}
	...
	if (rel == rLessEqual) {
		assert((vverif[0] == 25) && (vverif[1] == 13) && (vverif[2] == 11) && (vverif[3] == 4));
	}
	else {
		assert((vverif[0] == 4) && (vverif[1] == 11) && (vverif[2] == 13) && (vverif[3] == 25));
	}
}
...
void testAllExtended() {
	...
	testStack();
	testIteratorInBothDirections(rLessEqual);
	testIteratorInBothDirections(rGreater);
